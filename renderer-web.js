...
// === Web Excel Hook (non-invasive; uses your existing upload button) ===
(function(){
  if (!window._isWeb) return;
  ...
  function bindUploadBtn(){
    let btn = document.getElementById('btnUploadExcel')
          || document.querySelector('[data-action="upload-excel"]')
          || document.querySelector('#uploadExcel, #upload, .upload-excel, .btn-upload-excel');
    if (!btn){
      const cands = Array.from(document.querySelectorAll('button,[role="button"],.btn'));
      btn = cands.find(el => /بارگذاری|آپلود|Upload|Excel|اکسل|CSV/i.test((el.textContent||'').trim()));
    }
    if (btn && !btn.dataset._wiredXlsx){
      btn.addEventListener('click', ()=> fileInput.click());
      btn.dataset._wiredXlsx = '1';
    }
  }
  ...
  function applyRows(rows){
    if (typeof ingestRowsFromExcel === 'function') return ingestRowsFromExcel(rows);
    if (typeof loadExcelData      === 'function') return loadExcelData(rows);
    if (typeof applyExcelRows     === 'function') return applyExcelRows(rows);
    if (typeof importFromExcel    === 'function') return importFromExcel(rows);
    try { window.dataRows = Array.isArray(rows) ? rows : []; } catch(_){}
    try {
      const q = document.getElementById('search')?.value || '';
      if (typeof renderTable === 'function') renderTable(q);
      if (typeof updateTableCounters === 'function') updateTableCounters();
    } catch(e){}
  }
  ...
  function onFileChange(e){
    ...
    if (typeof XLSX === 'undefined'){
      alert('XLSX لود نشده (libs/xlsx.full.min.js)');
    ...
    r.onload = (ev)=>{
      try {
        ...
        applyRows(rows);
      } catch(ex){ console.error('Excel parse failed:', ex); alert('خواندن فایل ناموفق بود.'); }
  ...
  function wire(){
    bindUploadBtn();
    if (!fileInput.dataset._xlsx) { fileInput.addEventListener('change', onFileChange); fileInput.dataset._xlsx = '1'; }
  }
